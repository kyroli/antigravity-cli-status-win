...
- **Credential Storage & Switching**: Accesses active tokens using the Win32 `CredReadW` API. Handles interactive hot-switching and secure profile backup storage in the Windows Credential Manager utilizing the Win32 `CredWriteW` API under `gemini:antigravity:<alias>` targets with a fallback to local files, ensuring zero plaintext token exposure in configurations.
- **Quota Exhaustion Handling**: Triggers a high-visibility warning `[EXHAUSTED] (Switch via --config)` wrapping the widget in an OSC 8 terminal hyperlink when the remaining quota fraction drops to zero.
...
### 2. Theme & Profile Configuration (`~/.gemini/antigravity-cli/statusline/statusline.json`)

The application utilizes pre-configured defaults. Layout elements, padding, and state indicators are dynamically computed based on the active terminal column width.

Theme selection and saved account profile aliases are the configurable parameters.
...
- `--config`: Launches the interactive TUI configuration panel for layout visibility, theme switching, and multi-account profile management.
